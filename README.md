...
User uploads PDF  →  FastAPI (main.py)  →  Redis Queue  →  Celery Worker
                         |                                  |
                         | Returns job_id                   | Runs 4 AI agents
                         | immediately                      | (2–3 minutes)
                         v                                  v
                  SQLite Database (analysis.db)  ←────────  Saves result
                  
⚙️ How It Works

1️⃣ User Upload

• User uploads a financial PDF at http://localhost:8000
...
• Pushes task to Redis via Celery .delay()
...
• Removed sarcastic / irrelevant prompts
• Required “Not Found” if data missing
• Structured concise bullet-point outputs
• Added verification layer to validate financial keywords
